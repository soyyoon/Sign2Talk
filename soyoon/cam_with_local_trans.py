...
from translator import ko_sentence_from_kor_tokens
...
# 로컬 문장 생성
# ============================================================
def generate_sentence_locally(word_list):
    """GPT API 대신 translator.py의 로직을 사용하여 문장 생성"""
    if not word_list:
        return "단어가 없습니다."
    
    # 1. '01', '07' 같은 코드를 '안녕', '나' 같은 한글 단어로 변환
    korean_words = [KSL_LABELS_KR.get(word, word) for word in word_list]
    
    # 2. 로컬 규칙(조사 추가, 동사 변환 등)에 따라 문장 생성
    sentence = ko_sentence_from_kor_tokens(korean_words)
    
    # 만약 빈 결과가 나오면 단어만 나열
    if not sentence.strip():
        sentence = ", ".join(korean_words)
...
class SignLanguageRecognizer:
    ...
    def stop_recording_and_generate(self):
        ...
        # 수정된 부분: generate_sentence_with_gpt 대신 generate_sentence_locally 호출
        self.generated_sentence = generate_sentence_locally(self.word_list) 
        self.generating = False
        print(f"✅ 로컬 문장 생성 완료: {self.generated_sentence}\n")
